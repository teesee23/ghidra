...
public class DecompilerSugiyamaLayout extends AbstractFGLayout {
	...
	private void findLongestPath(FGVertex start, VisualGraph<FGVertex, FGEdge> jungGraph, Vector<FGVertex> longest) {


	}
	/* End Tom */

	public DecompilerSugiyamaLayout(FunctionGraph graph) {
		this(graph, true);
	}

	private DecompilerSugiyamaLayout(FunctionGraph graph, boolean initialize) {
		super(graph);
		if (initialize) {
			initialize();
		}
...
//		System.err.println(text);
	...
	protected AbstractVisualGraphLayout<FGVertex, FGEdge> createClonedFGLayout(
		...
		return new DecompilerSugiyamaLayout(newGraph, false);
	}
